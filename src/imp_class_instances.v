...
Section class_instances_assert_of.
  ...
  Lemma assert_of_mono P Q : (∀ v, P v ⊢ Q v) → assert_of (λ v, P v) ⊢ assert_of (λ v, Q v).
  ...
  Lemma combine_sep_assert_of_test P Q : ⊢ (assert_of (λ n, P)) -∗ (assert_of (λ n, Q)) -∗ assert_of (λ n, P ∗ Q).
...
Section class_instances_up1_down1.
  ...
  Instance from_sep_up1 : forall P Q, FromSep (⇑ (P ∗ Q))%I (⇑ P) (⇑ Q).
  Proof. intros; rewrite /FromSep -up1_sep //. Qed.
  
  Instance from_and_up1 : forall P Q, FromAnd (⇑ (P ∧ Q))%I (⇑ P) (⇑ Q).
  Proof. intros; rewrite /FromAnd -up1_and //. Qed.
  
  Instance from_exists_up1 : forall {T:Type} (P: T → assert),
    FromExist (∃ x, ⇑ (P x))%I (λ x, ⇑ (P x))%I.
  Proof. intros; rewrite /FromExist -up1_exist //. Qed.
  
  Instance into_wand_up1 p q (R P Q: assert) : IntoWand p q R P Q → IntoWand p q (⇑ R)%I (⇑ P)%I (⇑ Q)%I.
  ...
  Instance from_sep_down1 : forall P Q, FromSep (down1 (P ∗ Q))%I (down1 P) (down1 Q).
  Proof. intros; rewrite /FromSep -down1_sep //. Qed.
  
  Instance from_and_down1 : forall P Q, FromAnd (down1 (P ∧ Q))%I (down1 P) (down1 Q).
  Proof. intros; rewrite /FromAnd -down1_and //. Qed.
  
  Instance from_exists_down1 : forall {T:Type} (P: T → assert),
