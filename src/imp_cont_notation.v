...
From iris_imp_lang Require Export implang_cont.
...
Notation "'fn'  <{ ( ) s }> " := (Func [] [] s%S)
  (at level 200, s at level 200,
  format "'fn'  <{ ( ) '/  ' s '/' }> ") : func_scope.
Notation "'fn'  <{ ( a ) s }> " := (Func [] [a%binder] s%S)
  (at level 200, a at level 1, s at level 200,
  format "'fn'  <{ ( a ) '/  ' s '/' }> ") : func_scope.
Notation "'fn'  <{ ( a b .. c ) s }> " := (Func []
  (cons a%binder (cons b%binder .. (cons c%binder nil) ..)) s%S)
  (at level 200, a,b,c at level 1, s at level 200,
  format "'fn'  <{ ( a  b  ..  c ) '/  ' s '/' }> ") : func_scope.
...
Notation "'fn'  x <{ ( a b .. c ) s }> " := (Func [x%binder]
  (cons a%binder (cons b%binder .. (cons c%binder nil) ..)) s%S)
  (at level 200, x,a,b,c at level 1, s at level 200,
  format "'fn'  x  <{ ( a  b  ..  c ) '/  ' s '/' }> ") : func_scope.
Notation "'fn' x y .. z <{ ( ) s }> " := (Func (cons x%binder (cons y%binder .. (cons z%binder nil) ..))
  [] s%S)
  (at level 200, x,y,z at level 1, s at level 200,
  format "'fn'  x  y  ..  z  <{ ( ) '/  ' s '/' }> ") : func_scope.
Notation "'fn' x y .. z <{ ( a ) s }> " := (Func (cons x%binder (cons y%binder .. (cons z%binder nil) ..))
  [a%binder] s%S)
  (at level 200, x,y,z,a at level 1, s at level 200,
  format "'fn'  x  y  ..  z  <{ ( a ) '/  ' s '/' }> ") : func_scope.
