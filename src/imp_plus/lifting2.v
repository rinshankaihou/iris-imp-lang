...
Definition state_ctx s := (gen_heap_interp s.2 ∗
  ∃ ρ0 n, env_auth (<[n := s.1]>ρ0) ∗ own γ (●E n))%I.
...
Definition wp_sk E s k Q := (stack_top (stack_depth k) -∗
  ⎡wp NotStuck E (s, k) (λ _, ∃ n', own γ (◯E n') ∗ Q n')⎤)%I.
...
Lemma var_e : forall n x v s,
  stack_top n ∗ x ↦v v ∗ ⎡state_ctx s⎤ ⊢ ⌜s.1 !! x = Some v⌝.
Proof.
...
Lemma wp_adequacy Σ `{!gen_heapGpreS loc val Σ} `{!inG Σ (@envR val)} `{!invGpreS Σ} F s σ φ :
  (∀ `{!gen_heapGS loc val Σ} `{!envGS val Σ} `{Hinv : !invGS_gen hlc Σ},
